...
import br.com.cotiinformatica.api_pedidos.dtos.requests.ItemPedidoRequest;
...
public class PedidoService {
    ...
    public PedidoResponse criar(PedidoRequest request) {
        ...
        //Buscar no banco de dados o cliente informado no pedido
        var cliente = clienteRepository.findById(request.getClienteId())
                .orElseThrow(() -> new IllegalArgumentException("Cliente não encontrado."));

        //Criando um pedido
        var pedido = new Pedido();
        pedido.setClienteId(cliente.getId()); //Associando o cliente
        pedido.setDataPedido(LocalDateTime.now()); //Data e hora atual
        pedido.setValor(0.0); //Valor inicial

        //Capturar os itens do pedido
        var itensPedido = new ArrayList<ItemPedido>();
        for(var item : request.getItensPedido()) {

            //Buscar no banc de dados o produto informado no item do pedido
            ...
            //Somar o preço * quantidade no valor do pedido
        ...
        //Retornar os dados do pedido como resposta:
